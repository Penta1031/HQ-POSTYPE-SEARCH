...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (action === "set_visibility") {
      const visible = flag(payload.visible);
      const rows = await rest(filter, {
        method: "PATCH",
        body: JSON.stringify({
          admin_reviewed: visible,
          admin_reviewed_at: visible ? new Date().toISOString() : null,
        }),
      });
      return json({ ok: true, rows, visible });
    }
